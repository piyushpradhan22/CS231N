...
def softmax_loss_naive(W, X, y, reg):
    ...
    for i in range(num_train):
        f = score[i] - np.max(score[i]) # - term to avoid numerical instability
...
def softmax_loss_vectorized(W, X, y, reg):
    ...
    score_n = score - maxV.T # Avoiding numerical instability
    p_score = np.exp(score_n)
    probs = p_score / np.sum(p_score, axis=1, keepdims=True)
